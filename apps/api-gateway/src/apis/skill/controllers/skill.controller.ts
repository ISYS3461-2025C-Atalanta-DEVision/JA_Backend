...
export class SkillController {
  ...
  @ApiOperation({
    ...
    description: "Retrieve paginated list of skills",
  })
  @ApiQuery({
    name: "page",
    required: false,
  ...
  @ApiOperation({
    ...
    description: "Retrieve all skills belonging to a specific category",
  })
  ...
  @ApiOperation({
    ...
    description: "Create a new skill (requires API key or JWE auth)",
  })
  ...
  async update(@Param("id") id: string, @Body() updateDto: UpdateSkillDto) {
    return firstValueFrom(
      this.client
        .send({ cmd: "skill.update" }, { id, updates: updateDto })
        .pipe(
          timeout(5000),
          catchError((err) => {
            this.handleError(err);
  ...
  @ApiOperation({
    ...
    description: "Soft delete a skill (Admin only)",
  })
  @ApiParam({ name: "id", description: "Skill ID" })
  ...
  @Roles(Role.Admin)
  @ApiOperation({
    summary: "Hard delete skill",
    description: "Permanently delete a skill (Admin only)",
  })
  @ApiParam({ name: "id", description: "Skill ID" })
