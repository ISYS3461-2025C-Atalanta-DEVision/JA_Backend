...
export class EducationController {
  ...
  @Post()
  @ApiOperation({ summary: 'Create education', description: 'Create a new education profile (requires JWE auth)' })
    ...
    try {
      const result = await firstValueFrom(
        this.educationClient
          .send({ cmd: 'education.create' }, { createDto: createDto, applicantId: user.id })
          .pipe(
            timeout(5000),
            catchError((error) => {
  ...
  @Get(':id')
  @ApiKeyAuth()
  @ApiOperation({ summary: 'Get education by ID', description: 'Retrieve a single education by ID (requires API key or JWE auth)' })
  ...
  async findById(
    ...
    try {
      const result = await firstValueFrom(
        this.educationClient
          .send({ cmd: 'education.findById' }, { id })
          .pipe(
            timeout(5000),
            catchError((error) => {
              throw new HttpException(
  ...
  async findByApplicantId(
    ...
    try {
      const result = await firstValueFrom(
        this.educationClient
          .send({ cmd: 'education.findByApplicantId' }, { applicantId: applicantId })
          .pipe(
            timeout(5000),
            catchError((error) => {
              throw new HttpException(
  ...
  @Get()
  @ApiKeyAuth()
  @ApiOperation({ summary: 'Get all education', description: 'Retrieve paginated list of educations (requires API key or JWE auth)' })
  @ApiQuery({ name: 'page', required: false, type: Number, description: 'Page number (default: 1)' })
  @ApiQuery({ name: 'limit', required: false, type: Number, description: 'Items per page (default: 10)' })
  @ApiResponse({ status: 200, description: 'List of educations retrieved successfully' })
  @ApiResponse({ status: 401, description: 'Unauthorized' })
  @ApiResponse({ status: 500, description: 'Internal server error' })
  async findAll(
    @CurrentUser() user: AuthenticatedUser | undefined,
    ...
    const authType = (request as any).authType || 'jwt';
    const identifier = authType === 'jwt' && user ? user.email : 'API Key';
    this.logger.log(`${identifier} listing educations (page ${page})`);

    try {
      const result = await firstValueFrom(
        this.educationClient
          .send({ cmd: 'education.findAll' }, { page: Number(page), limit: Number(limit) })
          .pipe(
            timeout(5000),
            catchError((error) => {
              throw new HttpException(
  ...
  @Put(':id')
  @ApiOperation({ summary: 'Update education', description: 'Update an existing education profile (requires JWE auth)' })
  @ApiParam({ name: 'id', description: 'education ID' })
  ...
  async update(
    ...
    try {
      const result = await firstValueFrom(
        this.educationClient
          .send({ cmd: 'education.update' }, { id, updates: updateDto, userId: user.id })
          .pipe(
            timeout(5000),
            catchError((error) => {
              throw new HttpException(
  ...
  @Delete(':id')
  @ApiOperation({ summary: 'Delete education', description: 'Delete an education entry (requires JWE auth)' })
  @ApiParam({ name: 'id', description: 'education ID' })
