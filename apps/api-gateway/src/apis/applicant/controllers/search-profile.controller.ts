...
export class SearchProfileController {
  ...
  @ApiResponse({ status: 200, description: 'Search profile retrieved successfully' })
  ...
  @ApiOperation({
    ...
    description: 'Create or update the current user\'s search profile (requires JWT auth)',
  })
  @ApiBody({ type: UpsertSearchProfileDto })
  @ApiResponse({ status: 200, description: 'Search profile saved successfully' })
  ...
  async upsertProfile(
    ...
    try {
      const result = await firstValueFrom(
        this.applicantClient
          .send({ cmd: 'searchProfile.upsert' }, { applicantId: user.id, profile: dto })
          .pipe(
            timeout(5000),
            catchError((error) => {
              throw new HttpException(
  ...
  @ApiOperation({
    ...
    description: 'Deactivate the current user\'s search profile (requires JWT auth)',
  })
  @ApiResponse({ status: 200, description: 'Search profile deactivated successfully' })
  @ApiResponse({ status: 401, description: 'Unauthorized' })
  @ApiResponse({ status: 404, description: 'Search profile not found' })
  ...
  @ApiOperation({
    ...
    description: 'Activate the current user\'s search profile (requires JWT auth)',
  })
  @ApiResponse({ status: 200, description: 'Search profile activated successfully' })
  @ApiResponse({ status: 401, description: 'Unauthorized' })
  @ApiResponse({ status: 404, description: 'Search profile not found' })
