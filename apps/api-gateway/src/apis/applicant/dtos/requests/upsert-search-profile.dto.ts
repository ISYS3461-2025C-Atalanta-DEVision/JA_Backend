...
export class SalaryRangeDto {
  ...
  @ApiProperty({ example: 3000, description: 'Maximum expected salary', required: false })
  @IsOptional()
  @IsNumber()
  @Min(0)
  max?: number;

  @ApiProperty({ example: 'USD', description: 'Salary currency', default: 'USD' })
  @IsString()
  currency: string;
}
...
export class UpsertSearchProfileDto {
  ...
  @ApiProperty({ type: SalaryRangeDto, description: 'Expected salary range', required: false })
  @IsOptional()
  @ValidateNested()
  @Type(() => SalaryRangeDto)
  ...
  @ApiProperty({ example: true, description: 'Whether profile is active', required: false })
  @IsOptional()
  @IsBoolean()
  isActive?: boolean;
}
