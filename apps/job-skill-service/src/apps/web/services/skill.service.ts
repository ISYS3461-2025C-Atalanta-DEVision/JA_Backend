import { Injectable, NotFoundException, ConflictException, Logger, InternalServerErrorException, BadRequestException } from '@nestjs/common';
import { SkillRepository, Skill, JobCategoryRepository } from '../../../libs/dals/mongodb';
import { CreateSkillDto, UpdateSkillDto, SkillResponseDto } from '../apis/skill/dtos';
...
export class SkillService implements ISkillService {
  ...
  async create(createDto: CreateSkillDto): Promise<SkillResponseDto> {
    try {
      ...
      const jobCategory = await this.jobCategoryRepository.findById(createDto.jobCategoryId);
      if (!jobCategory) {
      ...
      if (error.code === 11000) throw new ConflictException('Skill with this name already exists');
  ...
  async findByCategory(categoryId: string): Promise<SkillResponseDto[]> {
    try {
      ...
      if (!jobCategory) {
        throw new NotFoundException(`Job category with ID ${categoryId} not found`);
      }

      const skills = await this.skillRepository.findByJobCategoryId(categoryId);
      return skills.map(s => this.toResponseDto(s));
    } catch (error) {
      this.logger.error(`Find skills by category failed for ${categoryId}`, error.stack);
      if (error instanceof NotFoundException) throw error;
      throw new InternalServerErrorException('Failed to fetch skills by category');
    }
  }
  ...
  async hardDelete(id: string): Promise<{ success: boolean; message: string }> {
    try {
      ...
      if (error instanceof NotFoundException) throw error;
      throw new InternalServerErrorException('Failed to permanently delete skill');
    }
  }
