...
export interface IOAuthAccountRepository extends IBaseMongoRepository<OAuthAccount> {
  ...
  findByApplicantIdAndProvider(applicantId: string, provider: string): Promise<OAuthAccount | null>;
  findByEmail(email: string): Promise<OAuthAccount[]>;

  /**
   * Store tokens for a applicant's provider account
   */
  storeTokens(applicantId: string, provider: string, tokenData: TokenData): Promise<void>;

  /**
   * Get refresh token hash for validation
   */
  getRefreshTokenHash(applicantId: string, provider: string): Promise<string | null>;
  ...
  updateProfile(provider: string, providerId: string, updates: Partial<OAuthAccount>): Promise<void>;

  /**
   * Update oauth account by applicantId and provider
   * Used to sync providerId (Firebase UID) and profile updates
   */
  updateByApplicantAndProvider(applicantId: string, provider: string, updates: Partial<OAuthAccount>): Promise<void>;
